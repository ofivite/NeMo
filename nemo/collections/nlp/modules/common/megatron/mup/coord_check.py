...
def get_optimizer_impl(optim_cfg):
    if (
            optim_cfg.name == 'adam'
            or optim_cfg.name in [
                'fused_adam',
                'distributed_fused_adam',
                'megatron_fused_adam',
            ]
            and not optim_cfg.get('adam_w_mode', True)
    ):
        optimizer_impl = 'adam'
    elif (
            ...
            and optim_cfg.get('adam_w_mode', True)
    ):
        optimizer_impl = 'adamw'
    elif optim_cfg.name == 'sgd':
        optimizer_impl = 'sgd'
    else:
        raise ValueError(f'optimizer {optimizer_impl} has not been implemented into muP yet')
    return optimizer_impl
...
def record_coords(records, width, modulename, t,
...
def _get_coord_data(models, dataloader, optcls, nsteps=3,
    ...
    Inputs:
        ...
        output_fdict, input_fdict, param_fdict:
            function dicts to be used in `record_coords`. By default, only `l1`
            ...
            for batch_idx, batch in enumerate(dataloader, 1):
                ...
                for name, module in model.named_modules():
                    ...
                    remove_hooks.append(module.register_forward_hook(
                        record_coords(df, width, name, batch_idx,
...
def get_coord_data(models, dataloader, optimizer='sgd', lr=None, mup=True,
    ...
    if mup:
        from nemo.collections.nlp.modules.common.megatron.mup.optim import MuAdam as Adam
        from nemo.collections.nlp.modules.common.megatron.mup.optim import MuAdamW as AdamW
        from nemo.collections.nlp.modules.common.megatron.mup.optim import MuSGD as SGD
    else:
